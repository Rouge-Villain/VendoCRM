...
export function prepareAnalyticsData(customers, opportunities, type) {
  switch (type) {
    case 'acquisition': {
      const acquisitionData = customers.reduce((acc, customer) => {
        if (customer.createdAt) {
          ...
          acc[monthYear].totalMachines += Array.isArray(customer.machineTypes) ? 
            customer.machineTypes.reduce((sum, machine) => sum + (machine.quantity || 1), 0) : 0;
...
export function exportAnalyticsData(customers, opportunities, type) {
